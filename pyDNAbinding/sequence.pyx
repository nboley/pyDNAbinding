import random
import cython
# from cpython.string cimport PyUnicode_AsEncodedString
...
for character_code in range(256):
    values = (encoding[str(chr(character_code))]
              if str(chr(character_code)) in encoding
...
def RC_map(base):
    ...
    if base == <char> 'N':
        return b'N'
    assert False, "Unrecognized base '{}'".format(base)


def reverse_complement(seq):
    rv = b"".join(RC_map(base) for base in seq[::-1])
    return rv
